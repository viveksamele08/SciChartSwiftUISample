...
//
// SCITranslateXyTransformationBase+Protected.h is part of SCICHART®, High Performance Scientific Charts
 ...
 */
@interface SCITranslateXyTransformationBase<__covariant T: SCIXSeriesRenderPassData *>()
 ...
 * The extensions in this header are to be used only by subclasses of `SCITranslateXyTransformationBase`.
 */
@interface SCITranslateXyTransformationBase<__covariant T: SCIXSeriesRenderPassData *>(Protected)
...
/**
 * Applies transformation using SCITranslateXyTransformationBase `transformValues:withOriginalCoordinates:andCurrentDelta:` method. `SCIFloatValues` to be supplied from inheritors.
...
/**
 * Transforms valuesToTransform based on original coordinates and currentDelta, to achieve translate transform.
 * @param currentDelta The current delta value, due to which values will be transformed.
